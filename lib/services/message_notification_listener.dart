...
class MessageNotificationListener {
  ...
  Map<String, Timestamp?> _lastMessageTimestamps =
      {}; // Track last message per chat room
  ...
  void startListening() {
    ...
    if (currentUser == null) {
      debugPrint(
        '[NotificationListener] No authenticated user, cannot listen for messages',
      );
      return;
    }

    debugPrint(
      '[NotificationListener] Starting to listen for messages for user: ${currentUser.uid}',
    );
  ...
  void _listenToChatRoomMessages(
    ...
    debugPrint(
      '[NotificationListener] Setting up listener for chat room: $chatRoomId',
    );
    ...
    final subscription = _firestore
        ...
        .snapshots()
        .listen(
          (messagesSnapshot) async {
            debugPrint(
              '[NotificationListener] Received ${messagesSnapshot.docs.length} messages in $chatRoomId',
            );
            ...
            for (var messageDoc in messagesSnapshot.docs) {
              ...
              debugPrint(
                '[NotificationListener] New message detected: $messageId from $senderId',
              );
          ...
          onError: (error) {
            debugPrint(
              '[NotificationListener] Error listening to messages in $chatRoomId: $error',
            );
            // Try to get more details about the error
            if (error.toString().contains('index')) {
              debugPrint(
                '[NotificationListener] Firestore index may be required. Check Firebase Console.',
              );
            }
          },
        );
  ...
  Future<void> _showMessageNotification({
    ...
    debugPrint(
      '[NotificationListener] Attempting to show notification for message from $senderEmail: $message',
    );
    ...
    try {
      await localNotifications.show(
        senderId.hashCode
            .abs(), // Use sender ID hash as notification ID (absolute value)
      ...
      debugPrint(
        '[NotificationListener] Notification shown successfully for $senderEmail',
      );
    } catch (e) {
